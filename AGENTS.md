...
- **每次新 Session 开始或 compact 之后，按这个顺序读，再动手**：
  1. **`ROADMAP.md` 顶部的「🔴 COMPACT 后从这里接续」** —— 恢复手册：有无实验在跑、
     **预先登记的停止规则**、环境事实（凭证/remote/门禁约束/证据目录）、下一步。
     compact 后**从这里接续**，不要凭印象猜进度。
  2. **`MEMORY.md` §0（能力判定表 + 当前阶段 + 不漂移协议）** —— 活记忆。
     教训不必通读：**先用 §0 之后的「索引：教训一句话」按 id 定位**，再看那一条
     （A 验证方法 / B 前端 / C 数据 / D 环境与工具 / E 提交卫生 / F 文档）。
  3. **`SPEC.md` 最近两段的实测结论** —— 当前是 §10.74–§10.81
     （pi-agent-core 协同 ADR → R1/R2a/R2b → M1/M2 → **M3 动作面盘点与探针证伪**）；
     更早的机制事实与统计量换代按需回查 §10.x。
...
7. **RL Harness 边界（项目所有者定的根本约束，不可越界）**：只向 agent 提供
   **①背景知识（游戏是什么、怎么操作）②世界事实与因果 ③交互协议**；
   **禁止注入任何策略/方法/该怎么做的提示**——智能必须由 agent 通过环境自己学出来。
   判据 J1–J4 与逐条对照的推导见 `docs/RL-HARNESS-SCOPE-AUDIT.md`（本铁律是规则，那份文档是论证）。
   推论：**工具必须真能改变世界、或明确拒绝**；工具对结果的陈述必须与世界一致。
8. **命令带副作用用本仓库脚本**，不裸跑破坏性 shell（尤其涉及 data-dir 清理）。
...
5. **改任何页面模板/样式/前端脚本后，必须跑两条命令**（2026-09-22 事故的根治）：
   - `pnpm run gate` —— **lint 对前端脚本已开 `no-undef`/`no-shadow`**。
     一次"未断言的批量替换"让一个常量没被声明，我整轮只跑 `vitest` 就把它发到了真机
     （图表每帧抛 ReferenceError，用户看到"闪烁 → 载入失败 → 成功但很长"循环）。
     一条 `eslint` 就能拦住它。**守卫存在 ≠ 守卫在循环里**（MEMORY D49）。
   - `pnpm run dashboard:smoke` —— 真实 Chrome + **全部级别控制台** + 用户可见元素
     + **图表健康**（`.uplot` 存在、宿主高度合理）。它会自带一个 dashboard（不拉 OpenTTD），
     跑完自动收尾。**"页面没报错"这种结论单测给不了**：前端脚手架禁止 jsdom/headless，
     没有 Alpine 求值就没有错误（D45）。
6. **症状是"变化/闪烁/往复"时，断言必须是时间序列**，不能是单帧快照。
   `dashboard:smoke` 现在在 10 秒内采样 20 次并断言高度**极差**；单帧断言对振荡**天然免疫**
   （用户描述的是"往复"，我证明的是"某一刻是对的"，于是四轮全绿而问题依旧）。
   **并且探针必须复现用户的浏览器条件**：除窗口尺寸/数据量外，还包括 **`devicePixelRatio`**
   （Retina = 2）。`dashboard:smoke` 用 `Emulation.setDeviceMetricsOverride` 固定 dpr=2；
   一个 dpr=1 的探针让"每帧乘 dpr"这类缺陷**永远测不出来**（MEMORY D48/D51）。
7. **看图**：修完前端问题后**截一张真实浏览器截图并自己看**。数值探针看不见
   "空白图表""标签被裁成 00000""幽灵占位文字"这类问题，而它们是用户唯一能看到的东西。
8. 页面脚本的**纯逻辑抽到 `<page>-view.js`** 并写单测；组件只留 IO 与命令式部件。
...
> **语言镜像**：`README.zh-CN.md` 是 `README.md` 的**翻译镜像**（无独立权威）。
> 两者**必须同步修改**；任何只改一份的提交视为文档漂移。中文版不得引入英文版没有的
> 事实或承诺。
