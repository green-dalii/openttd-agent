...
**Bridge GS 单机可以建完一条可运行的两站线路**（站点 / 路 / 车库 / 车 / 订单 / 启动，
全部引擎真值），但**当前还没量到交付 > 0**——`deliveredRun` 在真机里稳定为 0。
**不是"GS 做不到"，而是**两个新发现的、具体可命名的物理原因。

### 已被真机证实的事实（§10.90 的延伸）

1. **API 链路全通**（`probe_cm` 一次性建 1 段路 + 1 车厂 + 1 站 + 1 辆车 + 2 个订单 + 启动；
   钱 100000→70416 全部记在公司账上）。
2. **AB-4a 新建的状态机（`probe_route_cm`）也能跑完**（`/tmp/ab4n`）：同城内两站、18 格路、
   2 辆车、`orderCount:2 / ord0Station:true`、settle 60 tick 后**车真的动了**（`vehTile` 19880→19886，
   后续 19892→20407→19893，证明在两站之间行驶）。

### **两个新发现的物理原因**（不是"能力"问题）

1. **每 tick 异步建设（per-tick construction）**——`BuildRoad` 只是**安排**瓦片，
   真正的"路"要等下一两个 tick 才存在。第一版直接报 `run`，车永远停在 depot tile
   （`vehState:0` + `vehTile == depot`，利润 -66/-105 只付运行成本、不赚钱）。
   **修复**：`settle` 阶段，**观测到车动了**才进入 `run`（`stillForOneReport:false`），
   否则 2000 tick 后具名失败。这正是动作总线 I1（观测或具名拒绝）的探针侧落地。
2. **同城内两站路线在 OpenTTD 经济上是死胡同**——town 旅客的目的地是"别的镇"，
   不是同一座镇的另一个站；`waiting:0` 一直保持，90 运营日 `deliveredRun:0`。
   真机运行（`/tmp/ab4n`，settle 成功 + 2 辆车在 18 格路上跑了 90 天）确认这一点。

### 为什么**两条**都被记下来了

| 原因 | 影响范围 | 修复路径 |
|------|----------|----------|
| 每 tick 异步 | **任何** GS-only 路线都有 | 加 `settle`（已实现，可观测、可超时具名失败） |
| 同城经济死结 | 同城 fallback 模式 | 改成两城模式 → 但两城遇到 §10.90 里"寻路/地形/钱"三重困难 |

两城模式（在两座**距离 ≤ 25** 的镇之间建）的真机记录：`/tmp/ab4m` 建了 944 格路就
`ERR_NOT_ENOUGH_CASH`（钱剩 18）；`/tmp/ab4b` 第一个瓦片就 `ERR_AREA_NOT_CLEAR`。
**这是 executor 880 行寻路器存在的物理理由**——直线铺路对付不了真实地形。

### NEXT-4 的**部分**答案（诚实版）

GS-only 路线在**简单地形 + 短距离 + 双城**情况下**物理可行**（API + settle + 双城经济成立）。
但**任意地形 + 长距离**的寻路能力属于 A* 范畴，那是 executor 800+ 行的本来工作，
不是 GS-only 这次能替代的。

- 若接受"长线路仍由 executor 负责、GS 负责其它原语（建设施/经济/城镇/产业等）"——
  那么 GS + executor 双 VM **可以共存**，不需要退役 executor。
- 若硬要 GS-only 全包——需要把 A* 移植进 GS（≈ 800 行 + 跨 tick 状态机），
  收益不大（executor 已经能跑），**不建议做**。

### 守卫收获（D44）

发现表键未初始化导致整个 GS 静默死亡的 bug 时，新增的
"未初始化表键写入" 守卫**第一版也是空的**——`\\b`/`\\s` 在 `new RegExp(\`…\`)`
...
用户实测（`--serve` + 跑 agent）：
①"Result 图表栏抢占焦点，页面一直反复滚动无法控制"；②"图表长度一会过长一会恢复，
整页纵向极高（≥3000px），每个图表过窄"。

### 实测到的四个机制（都是量出来的，不是猜的）

| 指标 | 修复前 | 修复后 |
|------|--------|--------|
| 页面高度 | **7639px**（视口 900） | **4673px** |
| 滚动漂移（滚到 2000 后 25 秒内自己动的距离） | **131px** | **0** |
| uPlot 实例数（40 秒内） | **58** | **1** |
| DOM 变更 | 30 万/40s ≈ 7500/s | 见下（残余为性能项） |
| Stage views 面板 | **4317px**（占整页 57%） | **1081px** |
| Staged summaries 时间线 | **2254px**（46 条） | **300px**（8 条） |

#### 机制 1：图表**每帧销毁重建** → 版面抖动 → 滚动锚定把抖动转嫁给用户

调用侧是 `x-effect="drawCash()"`（每次遥测帧都跑），而适配层每次 `destroy` + `new uPlot`。
真机 40 秒内出现 **58 个 uPlot DOM 节点**、**30 万次 DOM 变更**。

**关键**：形状签名里含 series 的**显示名**，而 `cashSeries()` 的 name 是**公司名**——
执行器把相位写进公司名（`EX rd s4 r27 d24 p0 j101`），于是"名字"每帧都在变，
签名每帧都不同 → 每帧重建。
修法：签名只含**结构性**字段（高度 / 系列数 / 堆叠 / 调用方给的 `key`），
**显示名是数据不是形状**；`key` 取代了原来 `typeof format === "function" ? "fn"` 的写法
（切指标时 format 都是函数，靠类型根本分不出来——会把 cost 画成 money）。

#### 机制 2：**滚动锚定**（scroll anchoring）会跟用户抢滚动

文档高度每帧波动 131px，而用户的滚动位置**恰好也漂移 131px**——浏览器的"保持你看到的那一行
不动"在实时页面上必然变成抢滚动。
修法：`html { overflow-anchor: none }`（外加图表/列表容器级别）。
**滚动位置属于用户，不属于浏览器对锚点的猜测。**

#### 机制 3：两个列表**无限增长**

- Stage views：每到一个施工相位 +1 张（~283px/张）→ 24 张 = 4317px；
- Staged summaries：每段 +1 条 → 46 条 = 2254px。
修法统一为"**默认只渲染最新 N 条 + 展开其余**"（Stage 6 张 / 时间线 8 段）。
**上限是默认视图，不是丢数据**——按钮写明 "展开其余 N"，展开后全给。

#### 机制 4：`onSnapshot` 每帧换数组引用 → Alpine 整表重渲染

`this.recent = s.recent || []` 每次快照都产生新数组 → `x-for` 整表重渲染。
修法：`sameEventList()`（长度 + 末尾 seq/kind/文本长度等价即等价），等价时**保持同一引用**。

### 一处可读性修复（截图里发现的）

X 轴标签相邻重复（`1950-07, 1950-07, 1950-08, 1950-08, 1950-08 …`）：服务端每几秒推一个数据点，
而游戏日期只按**月**变。修法：相邻同名标签留空——重复刻度不提供信息，只是噪声。

### 追加（同日第二次排查）：真正的闭环是**宽度→高度**，而我第一次测错了尺寸

用户反馈"还是存在页面不受控制的跳动滚动 + 图表反复伸缩"。重测时**复现不出来**——
原因是我在 `1440×900` 下测，而**布局有 1312px 上限容器**：视口 1440 时内容宽度已经封顶，
滚动条出现/消失**不改变**内容宽度，耦合被掩盖。**探针必须复现用户的条件**，否则"稳定"是假稳定。
...
1. **`.snap-img` 的 `aspect-ratio: 1/1`** —— 快照高度**由宽度决定**，而全站**没有为滚动条预留宽度**
   → 滚动条出现/消失 → 宽度变 → 高度变 → 文档高度变 → 滚动条状态再变……**闭环**。
   修法：`html { scrollbar-gutter: stable; overflow-y: scroll }`（永久预留，断掉这一类）
   + `.snap-img { max-height: 260px }`（宽度超过 260 后高度固定）。
2. **uPlot 图例换行** —— legend 是图表宿主内的 DOM，文本变长会换到第二行 → 图表盒长高 ~20px
   → 文档高度变 → 滚动条 → 宿主宽度变 → 图例再换行……**第二条闭环**；
   而图例文本恰好曾是**公司名**（执行器把相位写进公司名，每帧都变），于是每帧触发。
   修法：`.chart-box .u-legend { flex-wrap: nowrap; overflow: hidden }`。

另：图表高度改为**取自视口高度**（`chartHeight(preferred, vh)`，0.30 比例夹在 160–260），
解决"子图过长（纵向）"——**刻意不取自宽度**，因为那正是闭环的来源。

### 追加（同日第三次排查）：轴宽、空状态、以及"数字全对但画面错"

用户复查"图表内容、比例错乱"。**三条关键发现全部来自我自己看真实浏览器截图**
（DOM 几何探针在同一时刻报告一切正常：canvas 1278×260、轴 52/1216、快照 362×362、控制台 0 报错）。

| 现象 | 真因 | 修法 | 真机结果 |
|------|------|------|----------|
| Y 轴标签被裁成 `00000` | 轴宽写死 **52px**，而 `£300.00k` 需 ~66px | 按实际标签文本测量（离屏 canvas `measureText`），轴宽进形状签名 | 轴宽 52 → **66px**，标签完整 |
| 图表是**没有轴的空盒子** | 零数据时 uPlot 什么都不画 | 空状态给**明确说明**且不构造 uPlot | 说明文案正确显示 |
| 说明文字**永远压在图上** | 加了空状态但**没写退场**（数据回来后占位未清） | 绘图前 `clearEmpty()` | 幽灵文字消失 |
| 标记与地图**错位** | 上一轮给 `.snap-img` 加的 `max-height: 260px` 破坏了正方形；而覆盖层用 `preserveAspectRatio="none"` 拉伸填盒、底图按宽度缩放 | 恢复正方形；尺寸限制改到**列宽** `minmax(280px, 380px)` | 快照 362×362，覆盖层同尺寸 |

**方法论结论**：数值探针回答"为什么"，截图回答"用户看到了什么"——**两者不可互相替代**。
本轮探针三次报告"正常"而画面是错的。故 `AGENTS.md` §5.2 增加第 6 条：
**修完前端问题必须截图自己看**。

### 未解决（如实记账，见 ROADMAP）

**残余 DOM 变更 ~17k/s**：`td.num`/`ol.events`/`span.tag`/`span.badge` 来自
**每帧重渲染的 200 行步骤表与事件列表**。这是**性能**问题（滚动漂移已为 0），
根因是 Alpine 在遥测帧上重算整表绑定，需要一次独立的分帧/增量渲染改造。
