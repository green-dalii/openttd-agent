...
| M2 | **目标与判据一致**（小） | 把**计分规则**作为事实写进 `SYSTEM_PROMPT`（"本项目按固定 horizon 内的每游戏日运货量评分"），或改经济参数让利润在窗口内可达。**只能改事实陈述，不得给策略** | 单测：提示词含计分事实、无策略词（既有守卫）；真机 1 局：agent 不再反复追求不可达目标（可由工具分布观察，不做因果断言） |
