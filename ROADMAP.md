...
| M3-2a | ✅ **每线路登记表 + 退役**（2026-09-19，SPEC §10.86） | 执行器跨 job 保留 `_routes[job]={vehicles,lead,depot,retired}`（切 job 时**先登记再重置**）；GS 新增 `retire_route` 命令 + `X:1` 标牌；执行器 `RetireRoute` 卖掉整条线路（**含头车**，普通缩编的下限 1 不适用），未知线路具名拒绝 `retire_unknown`；新增第 7 个工具 `retire_route`（sequential）。| ✅ 真机三次（`/tmp/m32*`）：**车队 8 → 2**（卖掉执行器登记的 6 辆；余下 2 辆是 prebuilt 场景自己的车，契约已写明）。✅ 守卫 3 条（登记表、退役含头车、未知拒绝）+ ✅ 两条**用重放证明过会失败**的守卫（array/table 用法矛盾、先用后声明）。🔵 遗留：**稀疏披露被采样通道盖掉**（见 M3-2b）|
| M3-2b | ✅ **披露通道可靠性**（2026-09-19，SPEC §10.87） | 公司名（`SetPhase`）是**单值、被采样**的通道，harness 每 500 ms 轮询，而执行器每轮都改写它——施工相位把 `retire…`/`fleetsend…` 这类稀疏披露**盖掉了**（SPEC §10.86 未结案）。已实现：`SetPhaseSticky(p, hold=15)`（事件粘住 N 轮）+ `WritePhase`（诊断可抢占，`exc:` 不被埋）。全部车队/退役事件走 sticky，守卫逐个检查＋重放证明。| ✅ **真机（`/tmp/m32e`）**：`fleet_wait6c1L1 → fleetg6L6C8 → fleet_wait3c6L6 → fleetsend3C8 → fleetsold1g0w0b0C2` 全部进日志，三操作生效（3→8→7→2），`exc:` 计数 0。**"工具能观测自身效果"在车队/退役动作上成立** |
| M3-2c | ✅ **车队请求对旧线路生效**（2026-09-19，SPEC §10.88） | `FleetRequests` 解析**所有** `V:` 标牌：当前 job 用活字段、已登记 job 用 `_routes[job]`（含 `applied`），未知/已退役分别具名拒绝（`fleet_unknown`/`fleet_retired`）。**这是 D20「同一请求重发 8 次」的根因修复**。| ✅ 真机（`/tmp/m32g`）：执行器在 **j102** 施工时处理 j101 的请求并正确执行缩编语义（4→3，`fleetsend1j101C9`），相位带 job id。✅ 守卫 3 条 + 重放证明。🔵 附带修复：`--second-route` 解析了但没进 `parseArgs` 返回值（第三次"声明了但没接线"）→ 新增 CLI 守卫（花括号配平抽取 + 重放证明）|
| M4a | ✅ **`recall` 按需检索**（2026-09-19，SPEC §10.89） | 第 9 个工具；`recallLessons`（纯函数）+ `renderRecallHit`（含实测读数）；`deps.recall` 受 `--no-memory` 门控（控制臂不得有检索，D4 第二问）；每次检索落审计；**存在性事实行**（库比注入集大时说明可用 recall 列出）。| ✅ 真机（`/tmp/m4a`）：模型**主动调用 2 次**、审计已记账（「记忆是否被用」第一次可查）。❌→✅ 第一版词不达意：子串匹配对模型的自然提问 0 命中（两条真实 query 为证）→ 改为**词级匹配 + 重叠排序**，并以真实库 + 真实 query 做 fixture 断言必须命中；契约（描述/schema/错误信息）同步改。✅ 接线/门控/记账守卫 3 条 + 重放证明 |
| M4b | 🔵 **选择压力**（小—中，当前） | 目前没有**任何一环按结果淘汰经验**（作废只来自模型自己的判断）：用 `recall`/注入的**使用记录**（审计）＋各局结果，做**观测性**的「哪条经验被用过、用过之后的成绩分布」，并在 verdict 里打印。⚠️ 不做因果主张（那需要 A/B，属 M5）；只做「可查」。 | 验收：①verdict 打印每条经验的使用次数与使用局的结果分布；②至少有一条经验被 ≥2 局用过（否则明确报「样本不足，不可判定」）|
...
- `/tmp/m31`→`m31e`→`m31f`：**M3-1 双向探针三轮**（探针把两个根因逼出来：`foreach` 取下标、卖车必须先停车库；`m31f` 实测扩容 3→14、**缩编 14→10**，SPEC §10.81/§10.84）
- `/tmp/g6`：G6 上下文峰值（两臂 5 188 / 7 324 tokens ⇒ 不需要压缩，SPEC §10.83）
- 更早的证据目录（**临时，环境重建即失效**）：`/tmp/pbcal`、`/tmp/cal900`、`/tmp/dm3`、`/tmp/f2`（凭据）。
  `/tmp/s0-v*` 已不存在——需要它的结论时看 `MEMORY.md`（D23）而不是路径。
...
| 记忆（跨局经验）是否提升成绩 | **未验证**（M5 未跑）。⚠️ 现在**至少可跑**：判据在 ≥180 天才会动（G7）|
| 是否存在**选择压力**（按结果淘汰经验） | **不存在**——作废只来自模型自己的判断（SPEC §10.85 B2）|
| 上下文是否需要压缩 | ✅ **已测**：峰值 5–7k tokens，离窗口 >10×，**不需要**（SPEC §10.83）|
...
### 5. 下一步（按序；权威依据是 SPEC §10.85 的复审与「🎯 M-PLAY」节）

**已完成**：M1 反思可诊断 ✅ · M2 计分规则作为事实 ✅ · M3 盘点 ✅ · M3-1 工具正名 ✅ ·
**M3-1b 执行器双向 ✅** · **G6 上下文计量（结论：不需要压缩）✅** · **G7 判据退化守卫 ✅**

1. **M3-2：补上最关键的那个动词——"退役"**（关掉亏损线路）。
   现状：能建线、能调车队（双向）、能暂停；**不能收摊**。对运输公司而言这是最有后果的动作之一。
   每项新动作都要过 D4（进台账 / 被 `--no-memory` 关 / 进对照判据）＋ 真机验证
   （工具必须能观测自身效果或明确拒绝——见 §10.81/§10.84 两次教训）。
2. **M4：检索 + 选择压力**。`recall` 工具（产出 `recallCalls`）＋ 自评事实（类型化消息）。
   ⚠️ 这是**新注入通道**，必须过 D4 三问。检索底座可考虑库的会话层，但按 §10.82 的门槛判断。
3. **M5：学习曲线**（`--mode sequential`，预登记见 R5 节）。
   ⚠️ 只能在 `--game-days ≥ 180` 上跑（G7 已在脚本里强制）；CV 0.57–0.79 ⇒ 检出 +40% 需 ~36/臂 ≈ 19 h。
