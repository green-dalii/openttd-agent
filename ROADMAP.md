...
| M3 | **动作面扩展**（中—大） | 先做 **GS 能力盘点**：现有 7 条命令里哪些可以低成本包成工具（如 `clear` = 退役线路？`status` = 查询？），再按"每局可行使的有后果决策数"排序补 1–3 条新变更命令（候选：退役/替换线路、车队减量、选货）。**每条新动作都要过 D4 三问 + 归因（工具必须观测自身效果或拒绝）** | 单测：新工具的拒绝路径与观测路径；真机：新动作在一局内被行使 ≥1 次且有账本记录；`loop-health` 报"有后果决策数/局"上升 |
