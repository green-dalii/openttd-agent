...
| M1 | **反思产出率**（小） | 同配置 1/2 局模型没调记录工具（已有 WARNING 指名）。先诊断：是模型选择还是提示词弱；若属后者，把"0 调用"视为失败并重试一次（限 1 次，记账） | 真机 2 局：`toolCalls>0` 且 `lessonsSaved>0`；账本出现 `reflectionRetries` 字段并在 verdict 披露 |
