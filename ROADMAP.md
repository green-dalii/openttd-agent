...
| M3 | 🔵 **动作面扩展**（中—大，当前） | **盘点 ✅**（SPEC §10.80）：环境只有 5 类标牌，运营期唯一杠杆是 `V:<count>`（下限 1、`V:0` = 无请求）。<br>**M3-1 ✅ 工具侧完成**：正名为 `set_route_vehicles`，契约写清双向/下限/零值，并回报**当前执行器阶段**。<br>**M3-1b 🔵（当前，需改执行器）**：真机探针（`--shrink-to`）**证伪**了"缩编可用"——`CheckAddVehicles()` 只在 `_stage == "done"` 时被调用，施工期请求**永不生效**（SPEC §10.81）。修法：锁存最新 `V:` 请求并在阶段允许时尽快应用（或把车队管理移出阶段机），然后**用同一条探针**证明双向都真的发生。<br>**M3-2（后做）**：整条线路退役 / 换车 / 选货——每项都要新 Squirrel + 真机验证。| M3-1：✅ 单测四条路径（扩容/缩编/下限/零值）+ 阶段回报。<br>M3-1b：单测（锁存语义）+ 真机 **扩容与缩编都发生**（`--add-vehicles N --shrink-to M` 的 `fleetObservedAfterShrink` 非空）|
