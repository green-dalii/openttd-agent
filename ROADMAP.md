...
| M3 | 🔵 **动作面扩展**（中—大，当前） | **盘点已完成**（SPEC §10.80，事实）：环境只有 5 类标牌，其中**运营期唯一的杠杆**是 `V:<count>`；它**本来就双向**（扩容=克隆、缩编=卖最新，下限 1，`V:0` 被当作"无请求"）。而工具叫 `add_vehicles`、契约只讲扩容 → **能力存在但没被暴露成动作**。<br>**M3-1（先做）**：把它正名为**设置车队规模**（如 `set_route_vehicles`），契约写清方向、下限、以及"0 不是退役"；缩编路径要有观测或拒绝。<br>**M3-2（后做，需新执行器能力）**：整条线路退役 / 换车 / 选货——每项都要新写 Squirrel + 真机验证。| M3-1：单测覆盖扩容/缩编/下限/零值拒绝四条路径；真机：**缩编**在一局内被行使 ≥1 次且车队数确实下降（观测，不靠自报）；`add_vehicles` 旧名在 CHANGELOG 记录为已更名（历史工具分布不可直接比较）|
