...
- [x] **已定位（2026-09-12）**：不是标牌可见性（心跳实测 `s3` = 3 个标牌全可见），
  而是**心跳从未响过** + **`FindSegment` 在单 tick 内跑 2000 次搜索**导致
  整个 AI 冻结。两者都已修，见 **SPEC §10.27**。
- [ ] **新发现**：执行器 script tick 供给不足（>6 秒/循环），施工慢得像卡住。待查脚本速度。

### 5. 执行器卡在 `road` 阶段（**根因已修，待验证能否走通**）

> **2026-09-12 更新**：卡住的根因已定位并修复（SPEC §10.27）—— 见 §4b。
> 修复后重试计数开始推进（`r0 → r1 → r2`）。**能否最终走通尚未验证。**
