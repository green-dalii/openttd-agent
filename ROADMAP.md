...
### 5. 执行器停在 `road` 阶段（**现在可观测，待解决**）

> **2026-09-12 更新**：执行器不再是"假死"，而是**真的在重试**（`rd s0 r0 → r7`，
> 心跳持续）。全世界冻结的根因已修（SPEC §10.28）。剩下的是**搜索本身不成功**：
> 分段探针 + Pathfinder.Road 在这张图上没找到可行路径。
> 现在它可观测、可迭代 —— 这是从"猜"到"调"的关键一步。
