...
> **RL Harness 范围（SPEC §10.22）**：初始化**只给背景知识**（这是什么游戏、怎么操作、
> 世界机制），一切智能行为由 agent 从环境交互中学。
> 判据：契约前提 ✅ / 世界事实与因果 ✅ / 交互协议 ✅ / **策略 ❌**。
> 完整审核（7 条注入通道逐一判定）见 [`docs/RL-HARNESS-SCOPE-AUDIT.md`](docs/RL-HARNESS-SCOPE-AUDIT.md)。

**跨局记忆默认关闭。** 只有显式传 `--inject-memory` 才会把过往局的经验交给 agent
（`DO:`/`AVOID:` 这类祈使句已删除，改为对过去的**陈述**）。
