...
1. **Y 轴标签被裁成 `00000`**（应为 `300000`）：轴宽写死 **52px**，而格式化后的标签
   `£300.00k` 需要 ~66px。**修法：轴宽按实际标签文本测量**（`axisSizeFor`，用离屏
   canvas 的 `measureText`；数值长大到需要更宽轴时通过形状签名触发重建）。
   真机结果：轴宽 52 → **66px**，标签完整可读。
2. **零数据时是一块没有轴的空盒子**（看起来像坏了）：改为**明确说明**
   （"no history yet — points appear as the run progresses"），并且**不构造 uPlot**。
3. **占位文字不退场**：数据回来后那句说明**永远压在图上**（真机截图中可见）。
   修法：绘图前 `clearEmpty()` 清掉占位；测试覆盖"进入 → 退出"。
4. **快照盒子被上一轮的 `max-height: 260px` 变成长方形** → 覆盖标记
   （`viewBox="0 0 100 100" preserveAspectRatio="none"` 会拉伸填盒）与底图
   （按宽度缩放）**错位**。修法：**恢复正方形**，尺寸限制改到**列宽**
   （`minmax(280px, 380px)`，列宽与视口无关 → 既限制尺寸又不产生闭环）。
   新增守卫：`.snap-img` **不得**出现 height/max-height（重放证明非空）。

另：确认 `--serve` 单独起**不拉 OpenTTD**，因此 `pnpm run dashboard:smoke` 改为
**自带一个 dashboard**（跑完自动收尾），图表断言只在**对局正在运行时**强制。

### Docs（把三次事故固化成契约与流程）

- `docs/DASHBOARD-UI.md` **§8 图表与页面的更新契约**：构造必须给宽度、形状签名不得含
  易变显示名、`key` 表达"画的是什么"、轴宽按标签测量、宿主预留图例、空状态必须退场；
  布局侧：高度不得依赖宽度（否则必须 `scrollbar-gutter: stable`）、快照必须正方形、
  列表默认最新 N、`x-for` 只迭代保证是数组的访问器、文档级 `overflow-anchor: none`。
- `AGENTS.md` §5.2 与 `CONTRIBUTING.md` §5：**改前端 ⇒ 跑 `gate` + `dashboard:smoke` +
  看一眼截图**（三条缺一不可）。理由：lint 能拦住"未声明常量"（`no-undef`），
  smoke 能拦住"控制台报错/图表没渲染"，截图能拦住"数字全对但画面错"。

### Fixed（图表"闪烁/载入失败/成功但很长"反复循环 —— 三个真因，2026-09-22）

用户实测："图表不是平滑刷新，而是闪烁、载入失败、成功（但很长）这样反复循环，
纵向大范围规律伸缩"。真机定位到**三个真因**（SPEC §10.95）：

1. **图表在 `width = 0` 下被构造**：uPlot 的样式是 `.uplot { width: min-content }`，
   宽度 0 时它按最小内容宽布局 → **图例被挤成一列** → 宿主从 120px 暴涨到 **622px**，
   等 `setSize` 才收回 271px。**修法：构造前就把宽度算好写进 opts**（不再有宽度 0 阶段）。
2. **`LEGEND_RESERVE_PX` 的声明根本没插进去**（一次未断言的批量文本替换静默失败）→
   每次 `mount` 抛 `ReferenceError` → 图表"失败 → 重试 → 失败"**循环**（真机 74 次报错，
   宿主高度轨迹 4061 → 5145，单次跳 1084px）。
3. **上一轮加的图例 CSS 对 uPlot 无效**：uPlot 的图例是 `<table>`（`.u-inline tr`），
   不是 flex 容器；已改成对表格真正有意义的规则，并改用
   **宿主预留一行图例高度**（`LEGEND_RESERVE_PX = 34`）让宿主高度与图例内容无关。

另：删掉复用判断里的**空 `catch {}`**（`shapeSignature` 是纯函数，不会抛）——空 catch 会瞒下真错误。

### Added（`dashboard:smoke` 的**图表健康**断言：图表坏了 = 红灯）

图表渲染失败时页面**不会明显报错**（`.uplot` 不存在、或宿主塌成 120px / 涨到 600px，
而面板看起来还在）。`pnpm run dashboard:smoke` 现在额外断言：每个图表宿主里**真的有
uPlot DOM**、宿主高度在 **140–420px**、宽度 ≥ 120px；不满足即失败（exit 1）。

### Docs（过程教训 D49：守卫存在 ≠ 守卫在循环里）

`no-undef` 早就在 eslint 里对前端脚本开启（起因 2026-09-11 的 TDZ 事故），重放证明它一行
命令就能拦住第 2 个 bug——**而这一轮我全程只跑 `vitest` + CDP 探针，一次没跑 `lint`/`gate`**。
故记录规则：改哪种语言就跑覆盖那种语言的守卫；批量替换必须断言锚点命中；
空 `catch {}` 是谎报器。

### Fixed（**仍是**页面跳动/子图反复伸缩的真凶：宽度→高度的闭环，2026-09-22 第二次排查）

用户复报"还是存在"。我上一轮的"验证通过"是**假稳定**：探针跑在 `1440×900`，
而布局有 **1312px 上限容器**——视口 1440 时内容宽度已封顶，滚动条出现/消失**不改变**内容宽度，
耦合被掩盖。换到 **1280** 立刻现形（快照宽 368→363，文档高度随之抖动）。

两条闭环（都不在上一轮的怀疑范围里）：

1. **`.snap-img { aspect-ratio: 1/1 }`**：快照高度**由宽度决定**，而全站**没有为滚动条预留宽度**
   → 滚动条出现/消失 → 宽度变 → 高度变 → 文档高度变 → 滚动条状态再变……**闭环振荡**。
   → `html { scrollbar-gutter: stable; overflow-y: scroll }`（永久预留，断掉这一类）
   + `.snap-img { max-height: 260px }`（宽度超过 260 后高度固定）。
2. **uPlot 图例换行**：legend 是图表宿主内的 DOM，换行会让盒子长高 ~20px → 文档高度变 →
   滚动条 → 宿主宽度变 → 图例再换行……**第二条闭环**；而图例文本恰好曾包含
   **每帧都变的公司名**（执行器把相位写进公司名），于是每帧触发。
   → `.chart-box .u-legend { flex-wrap: nowrap; overflow: hidden }`。

另修"Result 子图过长（纵向）"：图表高度改为**取自视口高度**
（`chartHeight(preferred, vh)`，0.30 比例夹在 160–260；720p 下由 291px 降到 ~247px），
**刻意不取自宽度**——宽度正是闭环的来源。

**定向实验证明闭环不可能再闭合**（视口宽度 ±15px = 滚动条宽度差）：

| | 1280 | 1265 | 差 |
|---|---|---|---|
| 快照 宽×高 | 368 × **260** | 363 × **260** | 宽变、**高不变** |
| 图表宿主 宽×高 | 1183 × **291** | 1168 × **291** | 宽变、**高不变** |
| **文档高度** | 4598 | 4598 | **0** |

### Fixed（Live 页运行期**滚动失控**与**版面失控**，2026-09-22）

用户实测：跑 agent 时"Result 图表栏抢占焦点、页面一直反复滚动无法控制"，
"图表长度一会过长一会恢复、整页纵向极高、每个图表过窄"。实测数据（SPEC §10.94）：

| 指标 | 前 | 后 |
|------|----|----|
| 页面高度 | 7639px | **4673px** |
| 滚动漂移（25 秒内自己动的距离） | **131px** | **0** |
| uPlot 实例数（40 秒） | **58** | **1** |
| Stage views 面板 | 4317px | **1081px** |
| Staged summaries 时间线 | 2254px | **300px** |

- **滚动锚定**（根因）：文档高度每帧波动 131px，浏览器"保持你看到的那一行不动"的善意
  在实时页面上就是**跟用户抢滚动** → `html { overflow-anchor: none }` +
  容器级 `overflow-anchor`（滚动位置属于用户）。
- **图表每帧重建**（根因）：`x-effect` 每帧触发，而"形状签名"里含 series 的**显示名**——
  那个名字是公司名，执行器把相位写进公司名（`EX rd s4 …`）→ 名字每帧变 → 每帧重建。
  改为只比较**结构性**字段（高度/系列数/堆叠/调用方 `key`），数据变化走 `setData`；
  新增 `key` 参数表达"这张图画的是什么"（切指标时 `typeof format === "function"` 恒相同，
  靠它区分会把 cost 画成 money）。
- **列表无限增长**：Stage views（每相位 +283px）、Staged summaries（每段 +49px）
  改为"默认最新 N + 展开其余"——**上限是默认视图，不是丢数据**。
- **每帧换数组引用**：`onSnapshot` 现在只在内容变化时替换 `recent`（`sameEventList`），
  避免 Alpine 每帧整表重渲染。
- **版面**：Stage views 面板跨整行、缩略图最小 320px（更大格、更少行）；
  KPI 瓦片最小 200px；图表宿主按调用方高度预留（销毁重建不再塌陷）。
- **可读性**：X 轴相邻重复标签留空（服务端每几秒推点、游戏日期按月变，
  过去会出现 `1950-07, 1950-07, 1950-08, …`）。
- 冒烟检查在本轮**当场抓到**自己引入的回归（`window.LiveView.sameEventList is not a function`，
  2 条异常）——§5.2 的自动化落点开始还债了。
...
- 新增 GS 命令 `probe_route_cm`：跨 tick 状态机（plan → stationA → stationB → road →
  depot → engine → buy → orders → **settle** → run），每阶段记引擎真值（站点瓦片、路格数、
  车库瓦片、引擎 id、车辆 id、订单数、订单 0 类型/目的地/flags、移动状态、利润、候客量、
  车库-路连通）。
- 新增 CLI 开关 `--probe-gs-route`（`--v02`）经**生产同一条** admin 通道触发，跑完测量
  60/90 运营日的 `deliveredRun`（与既有实验**同一仪表**，可比）。
- **真机结论（SPEC §10.92）**：GS 单机**可以**建完一条完整两站线路（站点/路/车库/车/
  订单/启动全部引擎真值），并通过新增的 **`settle` 阶段**等到车真的离开车库（这是
  AB-4a 的核心发现：**"启动成功" ≠ "在跑"**——OpenTTD 路/车的建设是**每 tick 异步**
  完成的，需要等）。但 `deliveredRun=0` 在多个种子下稳定复现：同城双站是 OpenTTD 经济
  死胡同（候客量持续为 0），双城模式遇到寻路/钱双重上限（executor 880 行寻路的物理理由）。
- 决定了** NEXT-4 的部分答案**：GS-only 路线在简单地形 + 短距离 + 双城下物理可行；
  **任意长距离**的寻路能力属 A* 范畴——**不建议移植**，executor **可保留**（GS 负责其它
  原语，如建设施/经济/城镇/产业）。
- 守卫 + 重放证明（D44）：新增"表键写入必须先初始化"守卫，**第一版就是空的**——`\\b`/`\\s`
  在 `new RegExp(\`…\`)` 模板里被当成字面字符，且自测用正则字面量通过了守卫却从未真正
  匹配过任何东西。重放证明抓到了这一点。修复：取最早绑定位置 + 解析别名
  `local pr = this._pr;` + 自测走与守卫本体同形态。
- 已知坏 API 列表（`KNOWN_BAD_CALLS`）新增 `GSMap.SLOPE_FLAT`：GS API 不暴露该常量；
  实测 `the index 'SLOPE_FLAT' does not exist`；用字面量 `0`（SLOPE_FLAT 的引擎值）+ 注释。
...
- 新增第 9 个工具 `recall`：检索**本局装载的那份记忆快照**（与开局注入同一份），
  返回带实测读数的事实行。匹配是**按词**的（任一词命中即算，命中更多词排更前），
  **不是语义检索**——契约（描述、schema、错误信息）三处一致写明。
- 每次检索写入审计（`{type:"recall", query, hits, ids}`）→「记忆到底有没有被用」
  第一次成为**可查事实**（此前只能看到「注入了几条」，那不等于「被用了」）。
- **`--no-memory` 下 `recall` 具名拒绝**（控制臂不得有检索，否则两臂之差不再是单一变量，D4）。
- 库比注入集大时，上下文多一行**存在性事实**（说明可以用 recall() 列出/检索）；
  不说出来，工具永远不会被调用，问题也就永远没有答案。
- 真机（`/tmp/m4a`）：模型**主动调用 2 次**；第一版两次都 0 命中（子串匹配对自然提问
  几乎不可能满足）→ 同轮改为**词级匹配 + 重叠排序**，并用**真实库 + 真实查询**做离线 fixture。
...
- 披露通道按**消费者**拆分：`SetPhase`（"现在在做什么"，可覆盖）、
  `SetPhaseSticky(p, hold=15)`（"发生过什么"，粘住 N 轮不被覆盖）、
  `WritePhase`（诊断写入，`exc:` 可抢占——错误不能被事件埋掉）。
- 动机：公司名是**单值、被采样**的通道（harness 每 500 ms 轮询），执行器每轮都改写它，
  施工相位把 `retire…`/`fleetsend…` 这类稀疏披露**盖掉**——动作生效、信号丢失
  （`/tmp/m32c`：三操作全生效、事件相位零条）。
- 真机验收（`/tmp/m32e`）：扩容/缩编/退役三操作全部生效（3→8→7→2），
  `fleet_wait/fleetg/fleetsend/fleetsold` **全部进日志**，`exc:` 计数 0。
- 途中又抓出并修掉两个同型缺陷：`SetPhaseSticky` 单参调用被 Squirrel 的
  **参数个数强校验**拒绝（`hold` 现有默认值，并由守卫钉住签名）；
  守卫各一条且都用**重放**证明过会失败。
- 静态守卫新增：array/table 用法矛盾、先用后声明、粘滞入口覆盖全部事件、
  `SetPhaseSticky` 签名（共 38 条 squirrel 守卫）。
...
- 遥测新增**单次请求峰值**：`usage.peakRequest = {tokens, turn}`，其中
  `tokens = input + cacheRead + cacheWrite`（一次请求真正送进模型的 prompt）。
  它随会话总计落进 metric 行与两个 verdict 的披露行。
- **为什么**：`totals.usage` 是**累计**量，回答不了"离模型上下文窗口还有多远"——
  而那是判断"是否需要上下文压缩"的唯一事实。此前我们的剪枝只是"最近 40 条消息、
  丢弃而非摘要"，且单局曾出现 148 万 token / 193 次工具调用（SPEC §10.67）。
- 缺测量时是 `not reported`，**不是 0**：旧 metric 行没有这个字段，读取侧按"未测量"处理；
  生产侧（`totalsFromTelemetry`）的接线由独立单测钉住——字段设成可选是**有意**的，
  因为把历史数据判成不合法并不能证明新值真的被填上。
- **实测结论（`/tmp/g6`，两臂）**：单次请求峰值 **5 188 / 7 324 tokens**（累计 24.8k/44.8k）
  ⇒ 离模型上下文窗口 >10× 余量，**不需要引入上下文压缩**；此前"单局 148 万 token"是
  **累计**量、来自一次失控决策（193 次工具调用），已被 R1 的每决策工具预算封住（本局 0 次拒绝）。

### Docs（第一性原理复检：能力面、空缺与协同结论，2026-09-19）

- SPEC §10.82：对着**安装产物**（`pi-agent-core@0.85.1` 的 `dist/*.d.ts` 导出表）逐项核对
  "库有什么 / 我们用了什么"，并回答四个问题（能否自主玩、能否自进化、机制是否完善、
  是否充分用库）。结论：`AgentHarness` 一整层（会话后端、**压缩**、**可查询历史**、
  技能/模板、运行期工具与模型切换）完全未用；其中压缩与检索是**真缺口**，
  而内置工具/执行环境是**应当不用**（会越界访问宿主）。
- 同节记录三条决策规则：引入库机制的门槛是"实测暴露的问题"；不引入会扩大 agent
  权限的库能力；能力审计必须对着安装产物、**不得把子系统折叠成一行**（MEMORY D35）。
- 修正 SPEC §10.74 里三行**已被 R1 推翻**的过时结论（工具执行顺序、`sessionId` 缓存、`thinkingLevel`）。

### Changed（M3-1：车队动作正名 + 真机证伪 + 探针，2026-09-19）

- 工具 `add_vehicles` → **`set_route_vehicles`**（GS 线上命令名不变）：契约现在写明
  **双向**（高于当前=克隆扩容、低于当前=卖最新克隆）、**下限 1**（头车永不卖，
  所以不能清空线路）、以及"0 被环境当作无请求"。
- 工具回报**当前执行器阶段**：真机探针证伪了我的源码推断——
  `CheckAddVehicles()` 只在 `_stage == "done"` 时被调用，**施工期的车队请求不会被读**。
  不把这条事实交出去，模型只会看到"请求了、什么都没变"，从而学会"请求车队没用"（D20）。
- 新增真机**双向探针** `--shrink-to N`（v02）：先等执行器 `done` 再发缩编请求并观测车队下降；
  `shrinkRequested` / `fleetObservedAfterShrink` 落进指标行。首次运行把
  "缩编可用"从**假设**变成**被量过的结果**：扩容 3→15 成功，**缩编未发生**（SPEC §10.81）。
- ⚠️ 因此"车队规模"目前**不是可靠可用的动作**；修执行器是 M3-1b（下一步）。
- `add_vehicles` 工具名历史不再存在：**工具分布指标 2026-09-19 前后不可直接比较**。

### Docs（M3 能力盘点：动作面到底有多大，2026-09-19）

- 新增 `SPEC.md` §10.80：把"环境允许 agent 做哪些动作"写成**事实表**——
  GS 7 条命令、执行器 5 类标牌（`S/E/D/W/V`），以及源码确证的三条关键约束：
  `V:<count>` **双向**（扩容克隆、缩编卖最新）、**下限 1**（头车永不卖 → 无法退役整条线路）、
  `V` 的 count `< 1` 被当作"无请求"而非"全卖"。
- 结论：运营期**唯一**的杠杆是车队规模，而它一直没有被正名暴露；
  再往上扩（退役/换车/选货）**需要新的执行器能力**，不是包一层工具能得到。
  M3 因此拆成 M3-1（正名暴露 + 缩编路径）与 M3-2（新增执行器能力）。

### Changed（M2：提示词如实说明计分规则，2026-09-19）

- `SYSTEM_PROMPT` 的目标不再宣称 "build profitable transport routes"（实验判据是运货量，
  而 106 局里 `income>0` 仅 9 局——目标通常不可达）。现在明确写出**计分规则事实**
  （按固定 horizon 内每游戏日运货量评分）与一条**会计事实**（施工期收入为负不代表线路没在工作）。
  这是事实陈述，不是策略：`assertNoStrategy` 守卫全绿。
- ⚠️ 提示词是全局改动：**2026-09-19 前后的行为指标（决策数/工具分布/车队规模）不可直接比较**。
